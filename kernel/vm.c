...
/*
 * Init kernel pagetable for the curr process
 */
pagetable_t
kpervminit()
{
  pagetable_t kpagetable = (pagetable_t)kalloc();
  memmove(kpagetable, kernel_pagetable, PGSIZE);
  kpagetable[0] = 0;
...
uvmcopy(pagetable_t old, pagetable_t new, pagetable_t knew, uint64 sz)
{
  ...
  for(i = 0; i < sz; i += PGSIZE){
    ...
    if(mappages(knew, i, PGSIZE, (uint64)mem, flags&~PTE_U) != 0){
        kfree(mem);
        goto err;
    }
  }
...
kvmcopy(pagetable_t old, pagetable_t new, uint64 sz)
{
  ...
  for(i = 0; i < sz; i += PGSIZE){
    ...
    if((mem = kalloc()) == 0)
      goto err;
    memmove(mem, (char*)pa, PGSIZE);
    if(mappages(new, i, PGSIZE, (uint64)mem, flags&~PTE_U) != 0){
      kfree(mem);
      goto err;
    }
  }
...
void
kvmclear(pagetable_t pagetable, uint64 va)
{
  pte_t *pte;
